...
choices_repeat = [[0, __("no"), ""], [1, _("daily"), __("day(s)")], [2, __("daily (workweek)"), __("workday(s)")],
                  [3, __("weekly"), __("week(s)")], [4, __("monthly"), __("month(s)")], [5, __("yearly"), __("year(s)")]]
        ...
        if old_task:
            ...
            if len(form.changed_data) > 0:
                ...
                for arg in form.changed_data:
                    ...
                    content['changes'].append(u"{arg} has changed from {old} to {new}".format(arg=arg.capitalize(), old=str(getattr(old_task, arg)), new=str(getattr(new_task, arg))))
        ...
        if self.id:
            ...
            if is_status_change and not is_estimate_time_change and not is_time_change:
                ...
                if old_task.status == 0 and self.status == 1:
                    TaskStatistics.add_statistics(user=self.owner, delta_tasks=1, estimate_time=zero_or_number(self.estimate_time),
            ...
            if is_estimate_time_change and not is_time_change and self.when_complete and not is_status_change:
                TaskStatistics.add_statistics(user=self.owner, delta_tasks=0, estimate_time=zero_or_number(self.estimate_time-old_task.estimate_time),
            ...
            if is_time_change and not is_estimate_time_change and self.when_complete and not is_status_change:
                TaskStatistics.add_statistics(user=self.owner, delta_tasks=0, estimate_time=0,
                                                spend_time=zero_or_number(self.time-old_task.time), date=self.when_complete)
...
class TaskStep(models.Model):
    ...
    author = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="step_author", blank=True, null=True,  on_delete=models.DO_NOTHING)
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="user_last_change", blank=True, null=True,  on_delete=models.DO_NOTHING)
    creation_date = models.DateTimeField(auto_now_add=True, editable=False, db_column='this_creation_date')
    modification_date = models.DateTimeField(editable=False, auto_now=True)
