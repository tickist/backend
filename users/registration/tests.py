...
class CheckEmailTest(TestCase):
    ...
    def test_check_email_fail(self):
        response = self.client.post(reverse("registration-check_email"), json.dumps({"email": self.john.email}),
                                    follow=True, content_type='application/json')
        result = json.loads(response.content.decode('utf-8'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(result['is_taken'], True)

    def test_check_json_without_email_field(self):
        response = self.client.post(reverse("registration-check_email"), json.dumps({"unknown": self.john.email}),
                                    follow=True, content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_check_email_ok(self):
        response = self.client.post(reverse("registration-check_email"), json.dumps({"email": "unknown@unkwnow.com"}),
                                    follow=True, content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
